...
function App() {
  ...
  const distance = Math.sqrt((Math.pow(foci2.x - foci1.x, 2)) + (Math.pow(foci2.y - foci1.y, 2)))
  ...
  const dist = (point: {x: number, y: number}, foci1: {x: number, y: number}, foci2: {x: number, y: number} ) => {
    const dist1 =  Math.sqrt((Math.pow(point.x - foci1.x, 2)) + (Math.pow(point.y - foci1.y, 2)))
    const dist2 =  Math.sqrt((Math.pow(point.x - foci2.x, 2)) + (Math.pow(point.y - foci2.y, 2)))
    return dist1 + dist2;
  }
  ...
  return (
    <>
      ...
      <input type="number" id="distance" name="distance" min="0" max="30" onChange={(e) => setRange(parseFloat(e.target.value))}/>
      <input type="number" id="foci1-x" name="foci1 - x" min="-20" max="15" onChange={(e) => setFoci1({...foci1, x: parseFloat(e.target.value)})}/>
      <input type="number" id="foci1-y" name="foci1 - y" min="-10" max="10" onChange={(e) => setFoci1({...foci1, y: parseFloat(e.target.value)})}/>
      <input type="number" id="foci2-x" name="foci2 - x" min="-20" max="15" onChange={(e) => setFoci1({...foci2, x: parseFloat(e.target.value)})}/>
      <input type="number" id="foci2-y" name="foci2 - y" min="-10" max="10" onChange={(e) => setFoci1({...foci2, y: parseFloat(e.target.value)})}/>
    </>
  );
}
